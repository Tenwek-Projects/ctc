...
@section('content')
    ...
    <div class="mb-4 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-600 shadow-sm sm:px-6">
        Members are listed by team group. Use the up/down controls to set order within each group — that order is used on the Specialists page and homepage.
    </div>
    ...
    <div class="rounded-xl bg-white border border-gray-200 shadow-sm overflow-hidden">
        ...
        @forelse($grouped as $groupKey => $group)
            <div @class(['border-t border-gray-200' => ! $loop->first])>
                <div class="bg-gray-50 px-4 py-2.5 sm:px-6">
                    ...
                    <p class="mt-0.5 text-xs text-gray-400">{{ $group['members']->count() }} {{ \Illuminate\Support\Str::plural('member', $group['members']->count()) }}</p>
                </div>
                                ...
                                <tr>
                                    <td class="align-middle">
                                        <div class="inline-flex items-center gap-1">
                                            <form action="{{ route('admin-dashboard.team-members.reorder', ['team_member' => $member->id], false) }}" method="post">
                                                @csrf
                                                @method('PATCH')
        ...
        @empty
            <div class="px-4 py-10 text-center text-sm text-gray-500 sm:px-6">
                No team members yet. <a href="{{ route('admin-dashboard.team-members.create') }}" class="text-admin-teal hover:underline">Add one</a>.
            </div>
        @endforelse
    </div>
@endsection
